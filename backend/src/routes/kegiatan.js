 ...
 *               waktu_menit: { type: integer, example: 90 }
 *               foto:
 *                 type: array
...
router.post("/", upload.array("foto"), async (req, res, next) => {
  try {
    ...
    const e = await store.addKegiatan(req.userId, {
      ...
      waktu_menit: parseInt(req.body.waktu_menit || "0", 10) || 0,
      foto_keys: fotoKeys,
    });
...
router.put("/:id", upload.array("foto"), async (req, res, next) => {
  try {
    ...
    if (req.body.waktu_menit !== undefined) patch.waktu_menit = parseInt(req.body.waktu_menit, 10) || 0;
